...
class VideoPlayer(VideoPlayerUI):
    ...
    def paintEvent(self, event):
        ...
        if self.rect and self.scaled_pixmap:
            # 獲取 scaled_pixmap 的尺寸
            pixmap_size = self.scaled_pixmap.size()
            # 計算紅框的相對坐標
            scaled_rect = QRect(
                self.rect.x() * pixmap_size.width() // self.video_label.width(),
                self.rect.y() * pixmap_size.height() // self.video_label.height(),
                self.rect.width() * pixmap_size.width() // self.video_label.width(),
                self.rect.height() * pixmap_size.height() // self.video_label.height()
            )
            # 複製圖片並畫紅框
